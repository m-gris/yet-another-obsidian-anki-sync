...
class GoldenTest extends munit.FunSuite:
  ...
  test("the golden's own coverage has not been hollowed out") {
    ...
    assertEquals(index.scan.specs.size, 56, "the fixture vault no longer produces 56 specs")
  ...
  /** NONE EMPTY, FIFTY-FIVE NOT — and the five that used to be empty are why the breadcrumb
    ...
    */
  test("no card has an empty Context, and all fifty-six carry a real breadcrumb") {
    ...
    assertEquals(nonEmpty.size, 56, "the number of cards carrying a real breadcrumb has changed")
