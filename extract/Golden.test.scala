...
class GoldenTest extends munit.FunSuite:
  ...
  lazy val actualFails: Vector[FailRecord] =
    ...
    index.scan.failures.map {
      ...
      case BuildFailure.KeyMisfiledInFile(noteId, _, _) =>
        FailRecord(escape("KeyMisfiledInFile"), escape(noteId.value))
      case BuildFailure.HeadingUnreadInFile(noteId, _, _) =>
        FailRecord(escape("HeadingUnreadInFile"), escape(noteId.value))
      ...
      case BuildFailure.ClozeBlockUnanchored(file, line, _) =>
        FailRecord(escape("ClozeBlockUnanchored"), escape(s"$file:$line"))
