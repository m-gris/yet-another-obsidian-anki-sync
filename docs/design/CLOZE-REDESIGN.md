...
- A **group** is one or more highlighted phrases sharing a label — `==<<1|radius>>== ==<<1|ulna>>==`
  is one group, and it blanks both phrases together as a single card. An unlabelled
  `==<<radius>>==` is a group of one. **One group makes one card.**
